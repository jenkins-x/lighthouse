...
| `foghorn.logLevel` | string | The logging level: trace, debug, info, warn, error, panic, fatal | `"info"` |
| `foghorn.maxConcurrentReconciles` | int | Value for `--max-concurrent-reconciles` on lighthouse-foghorn | `1` |
...
| `foghorn.skipTerminatedReconciles` | bool | When true, passes `--skip-terminated-reconciles` to lighthouse-foghorn (extra LighthouseJob watch predicates; default off matches legacy behavior) | `false` |
...
| `jenkinscontroller.podAnnotations` | object | Annotations applied to the jenkins controller pods | `{}` |
...
| `tektoncontroller.logLevel` | string | The logging level: trace, debug, info, warn, error, panic, fatal | `"info"` |
| `tektoncontroller.maxConcurrentReconciles` | int | Value for `--max-concurrent-reconciles` on lighthouse-tekton-controller (LighthouseJob and RerunPipelineRun controllers) | `1` |
...
| `tektoncontroller.skipTerminatedReconciles` | bool | When true, passes `--skip-terminated-reconciles` to lighthouse-tekton-controller (terminal activity / PipelineRun fast paths and extra LighthouseJob watch predicates) | `false` |
...
| `webhooks.httproute.annotations` | object | Webhooks httproute annotations | `{}` |
| `webhooks.httproute.enabled` | bool | Enable webhooks httproute | `false` |
| `webhooks.httproute.hosts` | list | Webhooks httproute host names | `[]` |
| `webhooks.httproute.parentRefs` | list | List of parent Gateways the HTTPRoute binds to TLS is configured on the parent Gateway listener, not the HTTPRoute | `[]` |
