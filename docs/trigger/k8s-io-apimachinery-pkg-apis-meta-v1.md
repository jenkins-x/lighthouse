...
| `matchExpressions` | [][LabelSelectorRequirement](./k8s-io-apimachinery-pkg-apis-meta-v1.md#LabelSelectorRequirement) | No | matchExpressions is a list of label selector requirements. The requirements are ANDed.<br />+optional<br />+listType=atomic |
...
| Stanza | Type | Required | Description |
|---|---|---|---|
| `key` | string | Yes | key is the label key that the selector applies to. |
...
| `values` | []string | No | values is an array of string values. If the operator is In or NotIn,<br />the values array must be non-empty. If the operator is Exists or DoesNotExist,<br />the values array must be empty. This array is replaced during a strategic<br />merge patch.<br />+optional<br />+listType=atomic |
