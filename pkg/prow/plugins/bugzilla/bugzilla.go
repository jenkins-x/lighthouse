...
import (
	...
	"github.com/jenkins-x/lighthouse/pkg/prow/bugzilla"
...
func handlePullRequest(pc plugins.Agent, pre scm.PullRequestHook) error {
	options := pc.PluginConfig.Bugzilla.OptionsForBranch(pre.PullRequest.Base.Repo.Namespace, pre.PullRequest.Base.Repo.Name, pre.PullRequest.Ref)
...
func digestPR(log *logrus.Entry, pre scm.PullRequestHook, validateByDefault *bool) (*event, error) {
	...
	var (
		...
		baseRef = pre.PullRequest.Ref
		number  = pre.PullRequest.Number
		title   = pre.PullRequest.Title
	)

	// Make sure the PR title is referencing a bug
	e := &event{org: org, repo: repo, baseRef: baseRef, number: number, body: title, htmlUrl: pre.PullRequest.Link, login: pre.PullRequest.Author.Login}
...
func digestComment(gc githubClient, log *logrus.Entry, gce github.GenericCommentEvent) (*event, error) {
	...
	if gce.Action != scm.ActionCreate {
		return nil, nil
	}
