...
// current state.
func takeAction(gc gitHubClient, cp commentPruner, l *logrus.Entry, org, repo string, pr scm.PullRequest, commitsMissingDCO []github.GitCommit, existingStatus string, hasYesLabel, hasNoLabel, addComment bool) error {
...
func handle(gc gitHubClient, cp commentPruner, log *logrus.Entry, org, repo string, pr scm.PullRequest, addComment bool) error {
...
func shouldPrune(log *logrus.Entry) func(scm.Comment) bool {
	return func(comment scm.Comment) bool {
		return strings.Contains(comment.Body, dcoMsgPruneMatch)
	}
}

func handlePullRequestEvent(pc plugins.Agent, pe scm.PullRequestHook) error {
...
func handlePullRequest(gc gitHubClient, cp commentPruner, log *logrus.Entry, pe scm.PullRequestHook) error {
	...
	shouldComment := pe.Action == scm.ActionSync ||
		pe.Action == scm.ActionOpen
...
func handleComment(gc gitHubClient, cp commentPruner, log *logrus.Entry, ce github.GenericCommentEvent) error {
	...
	if ce.IssueState != "open" || ce.Action != scm.ActionCreate || !ce.IsPR {
		return nil
	}
