...
import (
	...
	"github.com/jenkins-x/lighthouse/pkg/prow/plugins/golint/suggestion"
)
...
func modifiedGoFiles(ghc githubClient, org, repo string, number int, sha string) (map[string]string, error) {
	...
	for _, change := range changes {
		...
		case gfg.Match(change.Filename):
			continue
		case change.Status == scm.PullRequestFileRemoved || change.Status == scm.PullRequestFileRenamed:
			continue
		}
...
func newProblems(cs []scm.Review, ps map[string]map[int]lint.Problem) map[string]map[int]lint.Problem {
...
func handle(minimumConfidence float64, ghc githubClient, gc *git.Client, log *logrus.Entry, e *github.GenericCommentEvent) error {
	...
	if e.IssueState != "open" || !e.IsPR || e.Action != scm.ActionCreate {
		return nil
	}
	...
	return ghc.CreateReview(org, repo, e.Number, github.DraftReview{
		Body:     plugins.FormatResponseRaw(e.Body, e.Link, e.User.Login, response),
