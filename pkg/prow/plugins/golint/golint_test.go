...
func (g *ghc) ListPullRequestComments(org, repo string, number int) ([]scm.Review, error) {
...
func (g *ghc) GetPullRequest(org, repo string, number int) (*scm.PullRequest, error) {
	return &g.pr, nil
}
...
func TestLint(t *testing.T) {
	...
	for _, c := range gh.comment.Comments {
		pos := c.Position
		gh.oldComments = append(gh.oldComments, scm.Review{
			Path:     c.Path,
			Position: &pos,
			Body:     c.Body,
		})
		...
		},
		{
			...
			gh: &ghc{
				changes: []github.PullRequestChange{
					...
					{
						Filename: "bar.go",
						Status:   scm.PullRequestFileRemoved,
						Patch:    "@@ -1,5 +0,0 @@\n-package bar\n-\n-func Qux() error {\n-   return nil\n-}",
					},
				},
			},
		...
		},
		{
			...
			gh: &ghc{
				changes: []github.PullRequestChange{
					...
					{
						Filename: "bar.go",
						Status:   scm.PullRequestFileRenamed,
					},
				},
			},
			expectedModifiedFiles: map[string]string{
		...
		},
		{
			...
			gh: &ghc{
				changes: []github.PullRequestChange{
					{
						Filename: "qux.go",
						Status:   scm.PullRequestFileAdded,
						Patch:    "@@ -0,0 +1,5 @@\n+package bar\n+\n+func Qux() error {\n+   return nil\n+}",
					},
					{
						Filename: "bar.go",
