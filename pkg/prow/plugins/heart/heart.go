...
func handleIC(c client, adorees []string, commentRe *regexp.Regexp, ic scm.CommentEvent) error {
...
func handlePR(c client, pre scm.PullRequestHook) error {
	...
	for _, change := range changes {
		...
		if (filename == ownersFilename || filename == ownersAliasesFilename) && change.Additions > 0 {
			...
			return c.GitHubClient.CreateIssueReaction(
				pre.PullRequest.Base.Repo.Namespace,
