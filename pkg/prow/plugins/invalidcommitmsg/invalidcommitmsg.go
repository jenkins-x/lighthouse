...
import (
	...
	"github.com/jenkins-x/lighthouse/pkg/prow/plugins/dco"
)
...
func handle(gc githubClient, log *logrus.Entry, pr scm.PullRequestHook, cp commentPruner) error {
...
func hasPRChanged(pr scm.PullRequestHook) bool {
	switch pr.Action {
	case scm.ActionOpen:
		return true
