...
func handle(ghc githubClient, gc *git.Client, cp pruneClient, log *logrus.Entry, pre *scm.PullRequestHook) error {
	var (
	...
	if hasLabel && !existMergeCommits {
		...
		msg := plugins.FormatSimpleResponse(pre.PullRequest.Author.Login, commentBody)
