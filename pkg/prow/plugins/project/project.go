...
type githubClient interface {
	...
	GetRepos(org string, isUser bool) ([]scm.Repository, error)
...
func handle(gc githubClient, log *logrus.Entry, e *github.GenericCommentEvent, projectConfig plugins.ProjectConfig) error {
	...
	if maintainerTeamID == -1 {
		return gc.CreateComment(org, repo, e.Number, plugins.FormatResponseRaw(e.Body, e.Link, e.User.Login, notTeamConfigMsg))
	}
