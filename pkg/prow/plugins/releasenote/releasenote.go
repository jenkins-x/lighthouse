...
func handleIssueComment(pc plugins.Agent, ic scm.CommentEvent) error {
	return handleComment(pc.GitHubClient, pc.Logger, ic)
}

func handleComment(gc githubClient, log *logrus.Entry, ic scm.CommentEvent) error {
	// Only consider PRs and new comments.
	if !ic.Issue.IsPullRequest() || ic.Action != scm.CommentActionCreated {
		return nil
	}

	org := ic.Repo.Namespace
	repo := ic.Repo.Name
	...
	isMember, err := gc.IsMember(ic.Repo.Namespace, ic.Comment.User.Login)
	if err != nil {
		return err
	}
...
func handlePR(gc githubClient, log *logrus.Entry, pr *scm.PullRequestHook) error {
	// Only consider events that edit the PR body.
	if pr.Action != scm.ActionOpen && pr.Action != scm.ActionEdited {
		return nil
	}
	...
	if labelToAdd == ReleaseNoteLabelNeeded {
		...
		if containsNoneCommand(comments) {
			...
			comment := plugins.FormatSimpleResponse(pr.PullRequest.Author.Login, releaseNoteBody)
...
func clearStaleComments(gc githubClient, log *logrus.Entry, pr *scm.PullRequestHook, prLabels sets.String, comments []scm.Comment) error {
	...
	return gc.DeleteStaleComments(
		...
		pr.Number,
		comments,
		func(c scm.Comment) bool { // isStale function
...
func containsNoneCommand(comments []scm.Comment) bool {
...
func ensureNoRelNoteNeededLabel(gc githubClient, log *logrus.Entry, pr *scm.PullRequestHook, prLabels sets.String) {
	org := pr.Repo.Namespace
	repo := pr.Repo.Name
...
func prMustFollowRelNoteProcess(gc githubClient, log *logrus.Entry, pr *scm.PullRequestHook, prLabels sets.String, comment bool) bool {
	if pr.PullRequest.Ref == "master" {
		return true
	}
