...
func TestReleaseNoteComment(t *testing.T) {
	var testcases = []struct {
	...
	}{
		{
			...
			action:        scm.CommentActionCreated,
			commentBody:   "oh dear",
	...
	for _, tc := range testcases {
		...
		for _, l := range tc.currentLabels {
			ice.Issue.Labels = append(ice.Issue.Labels, scm.Label{Name: l})
		}
...
func newFakeClient(body, branch string, initialLabels, comments []string, parentPRs map[int]string) (*fakegithub.FakeClient, *scm.PullRequestHook) {
	...
	for _, comment := range comments {
		issueComments = append(issueComments, scm.Comment{Body: comment})
	}
	return &fakegithub.FakeClient{
			IssueComments: map[int][]scm.Comment{1: issueComments},
