...
func handleGenericComment(gc githubClient, isTrusted func(string) (bool, error), log *logrus.Entry, gce github.GenericCommentEvent) error {
	...
	if gce.IssueState != "open" || gce.Action != scm.ActionCreate {
		return nil
	}
	...
	if !trusted {
		return gc.CreateComment(org, repo, number, plugins.FormatResponseRaw(gce.Body, gce.Link, user, `Re-titling can only be requested by trusted users, like repository collaborators.`))
	}
	...
	newTitle := matches[1]
	if newTitle == "" {
		return gc.CreateComment(org, repo, number, plugins.FormatResponseRaw(gce.Body, gce.Link, user, `Titles may not be empty.`))
	}
