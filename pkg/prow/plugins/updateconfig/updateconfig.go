	...
	for _, change := range changes {
		...
		for _, ns := range append(cm.Namespaces) {
			...
			if key == "" {
				...
				if change.Status == scm.PullRequestFileRenamed {
			...
			if change.Status == scm.PullRequestFileRemoved {
...
func handle(gc githubClient, kc corev1.ConfigMapsGetter, defaultNamespace string, log *logrus.Entry, pre scm.PullRequestHook, config plugins.ConfigUpdater) error {
	...
	if !pr.Merged || pr.MergeSHA == nil || pr.Base.Repo.DefaultBranch != pr.Ref {
		return nil
	}
	...
	if err := gc.CreateComment(org, repo, pr.Number, plugins.FormatResponseRaw(pr.Body, pr.Link, pr.User.Login, msg)); err != nil {
