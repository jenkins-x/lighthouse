...
func ApplySecurityPolicyForLighthouseJob(c clientset.Interface, request *v1alpha1.LighthouseJob, repo ScmInfo, policiesNs string) error {
	policy, matchedAnyPolicy, err := findSecurityPolicyForRepository(c, repo.GetFullRepositoryName(), policiesNs)
	if err != nil {
...
func findSecurityPolicyForRepository(c clientset.Interface, repository string, policiesNs string) (v1alpha1.LighthousePipelineSecurityPolicy, bool, error) {
	policies, err := c.LighthouseV1alpha1().LighthousePipelineSecurityPolicies(policiesNs).List(context.TODO(), v1.ListOptions{})
	if err != nil {
		return v1alpha1.LighthousePipelineSecurityPolicy{}, false, errors.Wrapf(err, "API returned error while looking for LighthousePipelineSecurityPolicy")
	}
	...
	for _, currentPolicy := range policies.Items {
		...
		if err != nil {
			return v1alpha1.LighthousePipelineSecurityPolicy{}, false, errors.Wrapf(err, "panic! invalid repository pattern in LighthousePipelineSecurityPolicy of name %v", policy.Name)
		}
		if match {
	...
	if matched > 1 {
		...
		return v1alpha1.LighthousePipelineSecurityPolicy{}, false, errors.New(fmt.Sprintf("too many policies matched for repository %v", repository))
	}

	// no security policy matched, no restrictions to apply
	if matched == 0 {
		return v1alpha1.LighthousePipelineSecurityPolicy{}, false, nil
	}
